...
export function BundesligaSection() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchFootballStandings("Bundesliga"),
      fetchFootballFixtures("BUN", "scheduled", 5, today),
      fetchFootballFixtures("BUN", "finished", 4),
    ]).then(([s, u, r]) => {
      setStandings(s);
      setUpcoming(u);
      setRecent(r);
  ...
  return (
    <>
      ...
      {activeTab === "news" && <NewsTab competition="Bundesliga" accent={ACCENT} />}
