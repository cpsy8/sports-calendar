...
export function LaLigaSection() {
  ...
  useEffect(() => {
    fetchFootballStandings("La Liga").then((s) => {
      setStandings(s);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <>
      <div className="section-hero fade-in">
        ...
        <div className="hero-icon"><img src={competitionLogoUrl("LAL") ?? ""} alt="LaLiga" style={{ width: "100%", height: "100%", objectFit: "contain" }} /></div>
        <div className="hero-text">
          <h2>LA LIGA</h2>
          <p>Spain — 2025/26 Season</p>
        </div>
        {matchday !== null && (
          <div className="hero-badge" style={{ background: "#ff4b4420", color: ACCENT }}>
            Matchday {matchday}
          </div>
        )}
      </div>
      ...
      {activeTab === "fixtures" && (
        <FixturesTabPanel competitionShort="LAL" leagueCode={LEAGUE} accent={ACCENT} />
      )}
      ...
      {activeTab === "teams" && (
        <TeamsTab competitionShort="LAL" leagueCode={LEAGUE} accent={ACCENT} />
      )}
    </>
  );
}
